...
<div class="min-h-dvh bg-page font-body text-ink">
    ...
    <div class="grid gap-10 lg:grid-cols-2 lg:items-center">
      <div class="max-w-xl">
        ...
        <div class="mt-8 flex flex-wrap gap-4">
          <a
            href="#deploy"
            class="inline-flex items-center gap-2 rounded bg-mark bg-gradient-to-r from-cherry via-ember to-heat px-5 py-3 text-sm font-bold text-page hover:brightness-110"
          >
            Deploy with Docker
          ...
          <a
            ...
            class="inline-flex items-center gap-2 rounded border border-line bg-surface-1 px-5 py-3 text-sm font-medium text-ink hover:bg-surface-2 hover:border-line-strong"
          >
  ...
  <section class="border-y border-line bg-surface-1">
    <div class="mx-auto max-w-6xl px-6 py-12 sm:py-16">
      <p class="max-w-xl text-2xl font-semibold leading-tight text-white text-balance">
      ...
      <div class="mt-10 divide-y divide-line border-t border-line">
        <div class="grid gap-2 py-6 sm:grid-cols-[140px_1fr_auto] sm:items-center">
          <span class="font-display text-lg text-white">01 — RECEIVE</span>
          <span class="text-ink-2">A release webhook fires from GitHub or Forgejo, or your CI calls annalist directly.</span>
          <span class="text-xs tabular-nums text-ink-3">DEDUP BY EVENT HASH</span>
        </div>
        <div class="grid gap-2 py-6 sm:grid-cols-[140px_1fr_auto] sm:items-center">
          <span class="font-display text-lg text-white">02 — RESOLVE</span>
          <span class="text-ink-2">Per-repo settings merge with global defaults: tone, model, temperature, instructions.</span>
          <span class="text-xs tabular-nums text-ink-3">RESOLVED EFFECTIVE</span>
        </div>
        <div class="grid gap-2 py-6 sm:grid-cols-[140px_1fr_auto] sm:items-center">
          <span class="font-display text-lg text-white">03 — STRIKE</span>
          <span class="text-ink-2">An OpenAI-compatible LLM turns the commit log into release notes, then annalist publishes them back.</span>
          <span class="text-xs tabular-nums text-ink-3">IDEMPOTENT MARKER</span>
        </div>
      </div>
    </div>
  </section>
    ...
    <div class="grid gap-10 lg:grid-cols-2 lg:items-start">
      <div>
        ...
        <p class="mt-4 max-w-md text-ink-2">
          Pull the image, set <code class="rounded bg-surface-1 px-1 py-0.5 text-sm text-ink">ADMIN_TOKEN</code> and your LLM endpoint, and run. Then wire the examples pack to make every release ship with notes.
        </p>
        <a
          href="https://github.com/djdembeck/annalist/tree/main/examples"
          target="_blank"
          rel="noopener noreferrer"
          class="mt-6 inline-flex items-center gap-2 rounded border border-line bg-surface-1 px-4 py-2 text-sm font-medium text-ink hover:bg-surface-2"
        >
          <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      ...
      <div class="rounded border border-line bg-surface-1 p-4">
        <div class="mb-3 flex items-center justify-between">
          ...
          <button
            onclick={copyDocker}
            class="inline-flex items-center gap-1.5 rounded px-2 py-1 text-xs font-medium text-ink-2 hover:bg-control hover:text-ink"
            aria-live="polite"
          >
            {#if copied}
        ...
        <p class="mt-3 text-xs text-ink-3">
          Then run it with <code class="rounded bg-page px-1">ADMIN_TOKEN</code>, <code class="rounded bg-page px-1">LLM_BASE_URL</code>, and the platform credentials listed in the README.
        </p>
      </div>
    </div>
  </section>

  <footer class="border-t border-line bg-surface-1">
