<script lang="ts">
  ...
  async function runGenerate(r: Repo): Promise<void> {
    ...
    try {
      const result = await generate(r.platform, r.owner, r.repo, { to_tag: toTag, force: force[key] ?? false, publish: publish[key] ?? false });
...
<svelte:head>
  ...
  <meta name="description" content="Manage connected repositories and release-note generation." />
</svelte:head>
  ...
  <header class="section-head">
    ...
    <div class="section-head__actions">
      {#if loadState === "loading"}
        <button disabled class="btn btn-secondary" aria-label="Refreshing repository inventory">Refreshing…</button>
      ...
      {#each repos as r (rowKey(r))}
        ...
        <article class="panel repo-workpiece">
          <header class="repo-workpiece__header">
            <div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><span class="chip">{r.platform}</span><span class="status {r.enabled ? 'status--healthy' : 'status--quiet'}"><span class="signal-dot {r.enabled ? '' : 'signal-dot--muted'}" aria-hidden="true"></span>{r.enabled ? "Enabled" : "Disabled"}</span></div><h3 class="repo-workpiece__name">{r.owner}/{r.repo}</h3></div>
          ...
          <div class="repo-workpiece__actions">
            <button onclick={() => openGenerate(r)} aria-expanded={openPanel[key] === "generate"} aria-controls={openPanel[key] === "generate" ? `repo-generate-${key}` : null} class="btn btn-primary">{openPanel[key] === "generate" ? "Close generation" : "Generate note"}</button>
            <button onclick={() => openSettings(r)} aria-expanded={openPanel[key] === "settings"} aria-controls={openPanel[key] === "settings" ? `repo-settings-${key}` : null} class="btn btn-secondary">{openPanel[key] === "settings" ? "Close settings" : "Settings"}</button>
          </div>
          ...
          {#if openPanel[key] === "generate"}
            <section id="repo-generate-{key}" class="trace-panel" aria-label="Generate a note for {r.owner}/{r.repo}">
              ...
              <div class="mt-3 flex items-center gap-3">
                <label class="flex items-center gap-2 text-sm text-ink"><input type="checkbox" bind:checked={publish[key]} class="check-input" />Publish to release</label>
                <span class="text-xs text-ink-3">Preview-only by default. Check to write the note to the release.</span>
              </div>
              <div class="mt-4 grid gap-3 sm:grid-cols-2" aria-label="Generate mode for {r.owner}/{r.repo}">
                <button type="button" aria-pressed={!force[key] ? 'true' : 'false'} onclick={() => { force[key] = false; overwriteConfirm[key] = false; }} class="mode-choice {!force[key] ? 'mode-choice--active' : ''}"><span class="flex items-center gap-2 text-sm font-medium text-ink"><span class="signal-dot {!force[key] ? 'signal-dot--heat' : 'signal-dot--muted'}" aria-hidden="true"></span>Generate</span><span class="mt-1 text-xs leading-relaxed text-ink-3">Reuse the note already on file and never overwrite a release body edited by hand.</span></button>
                <button type="button" aria-pressed={force[key] ? 'true' : 'false'} onclick={() => { force[key] = true; overwriteConfirm[key] = false; }} class="mode-choice {force[key] ? 'mode-choice--danger' : ''}"><span class="flex items-center gap-2 text-sm font-medium text-ink"><span class="signal-dot {force[key] ? 'signal-dot--heat' : 'signal-dot--muted'}" aria-hidden="true"></span>Overwrite</span><span class="mt-1 text-xs leading-relaxed text-ink-3">Destroy the existing note body and generate a new one. This cannot be undone.</span></button>
              </div>
              {#if force[key] && overwriteConfirm[key]}
                <div class="mt-3 panel panel--error" role="alert">
                  <p class="text-sm text-alert">This will destroy the existing note for <strong>{generateTag[key] || "this release tag"}</strong>. Are you sure?</p>
              ...
              {/if}
              <div class="mt-4 flex flex-wrap items-end gap-3"><label class="field-group min-w-0 flex-1 sm:max-w-xs"><span class="field-group__label">Release tag</span><input bind:value={generateTag[key]} placeholder="v1.0.0" class="field" />{#if genTagError[key]}<span class="field-group__error" role="alert">{genTagError[key]}</span>{/if}</label><button onclick={() => runGenerate(r)} disabled={pending[key]?.generating || (force[key] && overwriteConfirm[key])} class="btn btn-primary w-full sm:w-auto">{pending[key]?.generating ? "Generating…" : publish[key] ? "Generate & publish" : "Preview"}</button></div>
              <div aria-live="polite" class="mt-4">{#if genError[key]}<p class="text-sm text-alert" role="alert">{genError[key]}</p>{/if}{#if notesOut[key]}<p class="mt-1 text-sm text-ok">Note generated.</p><div class="mt-4"><p class="trace-label">Generated note / release proof</p><pre class="note-paper mt-2 max-h-96 overflow-auto">{notesOut[key]}</pre></div>{/if}</div>
            </section>
          {/if}
        </article>
      {/each}
