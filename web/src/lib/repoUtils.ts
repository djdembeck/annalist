...
/** Classify a release's notes state for display. */
export function releaseNotesState(
  body: string | null | undefined,
  generatedMarker = "<!-- generated by annalist -->",
): "none" | "generated" | "manual" {
  const b = (body ?? "").trim();
  if (!b) return "none";
  return b.includes(generatedMarker) ? "generated" : "manual";
}

/** Best-effort local date label for an RFC3339 timestamp. */
export function releaseDate(iso: string | null | undefined): string {
  if (!iso) return "";
