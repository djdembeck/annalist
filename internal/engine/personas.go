...
When the release tag has the form <prefix>/vX.Y.Z, report only the commits that change the artifact that prefix names; commits that touch other artifacts, or only shared tooling, CI, release automation, or repository-wide documentation, are out of scope and omitted entirely — including release-pipeline work that cut the tag itself. Report every in-scope commit in the log exactly once. Write one bullet per in-scope commit. Never merge two distinct changes into one bullet. Never omit an in-scope commit. Never invent a change, component, or behavior not present in the log. Keep the specific nouns and behaviors of each change intact. Your tone may alter phrasing, but never which facts appear or how many changes are reported.`
