...
const forgejoPayload = `{"action":"created","release":{"id":42,"tag_name":"v0.2.0","draft":false},"repository":{"full_name":"djdembeck/milmus","name":"milmus","owner":{"login":"djdembeck","username":"djdembeck"}}}`
...
func TestForgejoWebhookSignatureAndDispatch(t *testing.T) {
	...
	t.Run("valid X-Gitea-Signature dispatches release event", func(t *testing.T) {
		...
		if gn.Tag != "v0.2.0" || gn.Owner != "djdembeck" || gn.Repo != "milmus" {
...
func TestForgejoWebhookPreDispatchRejections(t *testing.T) {
	...
	t.Run("missing owner is ignored with 200", func(t *testing.T) {
		noowner := bytes.Replace(good, []byte(`"full_name":"djdembeck/milmus"`), []byte(`"full_name":""`), 1)
