...
type Release struct {
	...
	Draft       bool
	CreatedAt   time.Time
	PublishedAt *time.Time // nil for unpublished drafts
}
...
// Marker is the HTML comment appended to every release body this app
// publishes; callers use it to recognize annalist-owned notes.
const Marker = marker

// SortReleases orders a release list in place for display: published first
// (descending publication time), then drafts (descending creation time).
// The sort is stable so releases sharing an effective time keep the order the
// platform returned them in. Shared by the github and forgejo clients so the
// ordering contract lives in one place.
func SortReleases(releases []Release) {
	sort.SliceStable(releases, func(i, j int) bool {
		if releases[i].Draft != releases[j].Draft {
			return !releases[i].Draft
		}
		t := releases[i].PublishedAt
		if t == nil {
			t = &releases[i].CreatedAt
		}
		u := releases[j].PublishedAt
		if u == nil {
			u = &releases[j].CreatedAt
		}
		return t.After(*u)
	})
}
...
func (p *Pipeline) GenerateNotes(ctx context.Context, spec Spec, opts Options) (string, error) {
	...
	// Auto-resolve the release ID so manual generation joins the same
	// generated_notes store as webhooks (which carry the platform-qualified ID).
	// This must precede the inflight lock: with an empty ReleaseID the lock
	// key would otherwise coarsen to platform-wide ("platform/"), serializing
	// unrelated manual runs on the same platform.
	if spec.ReleaseID == "" {
		rel, err := platform.GetReleaseByTag(ctx, spec.Owner, spec.Repo, spec.ToTag)
		if err != nil {
			return "", fmt.Errorf("pipeline: fetch release: %w", err)
		}
		spec.ReleaseID = spec.Platform + ":" + strconv.FormatInt(rel.ID, 10)
	}
